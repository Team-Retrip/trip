...
public class ItineraryQuerydslRepository implements ItineraryQueryRepository {
    ...
    public Page<ItineraryResponse> findItineraries(UUID tripId, Pageable page) {
        List<Itinerary> result = query.selectFrom(itinerary)
                .leftJoin(itinerary.itineraryDetails.values, itineraryDetail).fetchJoin()
        ...
        //Transform 필요
        List<ItineraryResponse> itineraries = result
                .stream()
                .map(i -> new ItineraryResponse(
                        ...
                        i.getItineraryDetails() != null ?
                                i.getItineraryDetails()
                                        .getValues()
                                        .stream()
                                        .map(id -> new ItineraryResponse.ItineraryDetailResponse(
                                                id.getId(),
                                                id.getDescription(),
                                                id.getPrice(),
                                                id.getLocationId()
