...
class InvitationServiceTest extends BaseInvitationServiceTest {
    ...
    void 여행_초대를_생성한다() {
        ...
        createLeader(TRIP_ID, LEADER_ID);

        TripInvitationsCreateRequest request =
                new TripInvitationsCreateRequest(LEADER_ID, List.of(정수_ID, 홍석_ID, 준호_ID));
    ...
    void 여행_초대_목록을_조회한다() {
        ...
        Page<InvitationsResponse> invitations =
                invitationService.getTripInvitations(
                        TRIP_ID, LEADER_ID, INVITED.name(), pageable, DATE, "desc");
    ...
    void 사용자의_여행_초대_목록을_조회한다() {
        ...
        createLeader(tripId1, LEADER_ID);

        UUID tripId2 = UUID.randomUUID();
        tripRepository.save(createTrip(tripId2));
        createLeader(tripId2, LEADER_ID);

        UUID tripId3 = UUID.randomUUID();
        tripRepository.save(createTrip(tripId3));
        createLeader(tripId3, LEADER_ID);

        invitationService.createInvitations(
                tripId1, new TripInvitationsCreateRequest(LEADER_ID, List.of(홍석_ID)));
        invitationService.createInvitations(
                tripId2, new TripInvitationsCreateRequest(LEADER_ID, List.of(홍석_ID)));
        invitationService.createInvitations(
                tripId3, new TripInvitationsCreateRequest(LEADER_ID, List.of(홍석_ID)));

        Pageable pageable = PageRequest.of(0, 10);

        // when
        Page<MemberInvitationResponse> invitations =
                invitationService.getMemberInvitations(
                        홍석_ID, INVITED.name(), pageable, DATE, "desc");
    ...
    void 초대를_수락하면_여행_멤버가_된다() {
        ...
        Trip trip = createTrip(TRIP_ID);
        tripRepository.save(trip);
        createLeader(TRIP_ID, MEMBER_ID);
