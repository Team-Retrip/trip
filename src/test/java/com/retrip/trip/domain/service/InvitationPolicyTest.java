...
class InvitationPolicyTest {
    ...
    void 리더가_아닌_멤버가_사용자를_초대하면_예외가_발생한다() {
        ...
        Participant participant = Participant.create(trip.getId(), 혁진_ID, ParticipantRole.PARTICIPANT)
    ...
    void 이미_여행_멤버인_사용자를_초대하면_예외가_발생한다() {
        // given
        Trip trip = createTrip(TRIP_ID);
        trip.addParticipant(Participant.createTripParticipant(혁진_ID, trip));
        trip.addParticipant(Participant.createTripParticipant(지수_ID, trip));
    ...
    void 여행이_최대인원이_채워진_경우_수락되지_않는다() {
        ...
        trip.addParticipant(Participant.createTripParticipant(혁진_ID, trip));
        trip.addParticipant(Participant.createTripParticipant(지수_ID, trip));
        trip.addParticipant(Participant.createTripParticipant(준호_ID, trip));
